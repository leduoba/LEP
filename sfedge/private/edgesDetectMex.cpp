...
// [E1,E2,E3,E4] = mexFunction(model,I,chns,chnsSs) - helper for edgesDetect.m
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  // create outputs zhaoqy
  pl[0] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *E1 = (float*) mxGetData(pl[0]);
  pl[1] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *E2 = (float*) mxGetData(pl[1]);
  pl[2] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *E3 = (float*) mxGetData(pl[2]);
  pl[3] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *E4 = (float*) mxGetData(pl[3]);

  pl[4] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *EN1 = (float*) mxGetData(pl[4]);
  pl[5] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *EN2 = (float*) mxGetData(pl[5]);
  pl[6] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *EN3 = (float*) mxGetData(pl[6]);
  pl[7] = mxCreateNumericArray(3,outDims,mxSINGLE_CLASS,mxREAL);
  float *EN4 = (float*) mxGetData(pl[7]);
  memset(EN1, 0, sizeof(float) * outDims[0] * outDims[1] * outDims[2]);
  memset(EN2, 0, sizeof(float) * outDims[0] * outDims[1] * outDims[2]);
  memset(EN3, 0, sizeof(float) * outDims[0] * outDims[1] * outDims[2]);
  memset(EN4, 0, sizeof(float) * outDims[0] * outDims[1] * outDims[2]);

  uint32 *ind = new uint32[indDims[0]*indDims[1]*indDims[2]];
  ...
  // computed sharpened edge maps, snapping to local color values
    ...
    const int g=gtWidth; uint16 N[4096*4], NZ[4096*2], CZ[4096*4]; 
    for( int c=0; c<g; c++ ) for( int r=0; r<g; r++ ) {
      int i=c*g+r; uint16 *N1=N+i*4, *NZ1=NZ+i*2, *CZ1=CZ+i*4;
      N1[0] = c>0 ? i-g : i; N1[1] = c<g-1 ? i+g : i;
      N1[2] = r>0 ? i-1 : i; N1[3] = r<g-1 ? i+1 : i;

	  NZ1[0] = (c<g-1 && r<g-1)? i+g+1 : i;
	  NZ1[1] = (c>0 && r<g-1) ? i-g+1 : i;

	  CZ1[0] = c<g-1 ? 1 : 0;
	  CZ1[1] = r<g-1 ? 1 : 0;
	  CZ1[2] = (c<g-1 && r<g-1)? 1 : 0;
	  CZ1[3] = (c>0 && r<g-1) ? 1 : 0;
    }
    #ifdef USEOMP
    #pragma omp parallel for num_threads(nThreads)
    ...
    for( int c=0; c<w1; c++ ) for( int r=0; r<h1; r++ ) {
      for( int t=0; t<nTreesEval; t++ ) {
        ...
        int m = nSegs[k];
		if( m==1 ) {
			int offset = c*stride*h2 + r*stride;
			float *EN11 = EN1 + offset;
			float *EN21 = EN2 + offset;
			float *EN31 = EN3 + offset;
			float *EN41 = EN4 + offset;
			for(int yz = 0; yz < gtWidth; yz++) {
				for (int xz = 0; xz < gtWidth; xz++) {
					int i = xz * gtWidth + yz;
					uint16 *CZ1=CZ+i*4;
   					EN11[eids[i]] += CZ1[0]; EN21[eids[i]] += CZ1[1];
					EN31[eids[i]] += CZ1[2]; EN41[eids[i]] += CZ1[3];
				}
			}
			continue;
		}
        uint8 S[4096]; memcpy(S,segs+k*g*g, g*g*sizeof(uint8));
...
///*
		for(int yz = 0; yz < gtWidth; yz++) {
			for (int xz = 0; xz < gtWidth; xz++) {
				int i = xz * gtWidth + yz; uint8 s = S[i];
				uint16 *N1=N+i*4, *NZ1=NZ+i*2;
				if( s!=S[N1[1]] ) E11[eids[i]]++;
				if( s!=S[N1[3]] ) E21[eids[i]]++;
				if( s!=S[NZ1[0]] ) E31[eids[i]]++;
				if( s!=S[NZ1[1]] ) E41[eids[i]]++;
				uint16 *CZ1=CZ+i*4;
				EN11[eids[i]] += CZ1[0]; EN21[eids[i]] += CZ1[1];
				EN31[eids[i]] += CZ1[2]; EN41[eids[i]] += CZ1[3];
			}
...
/*
		for( int b=b0; b<b1; b++ ) {
          ...
          if( s!=S[NZ1[0]] ) E31[eids[i]]++;
          if( s!=S[NZ1[1]] ) E41[eids[i]]++;
		  EN11[eids[i]]++; EN21[eids[i]]++;
		  EN31[eids[i]]++; EN41[eids[i]]++;
        }
//*/
      }
    }
